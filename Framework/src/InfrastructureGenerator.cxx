    ...
    } else // TaskLocationSpec::Remote
    {
      ...
      switch(taskSpec.dataSource.type) {
        case DataSourceType::DataSamplingPolicy:
          samplingPoliciesUsed.insert(taskSpec.dataSource.typeSpecificParams.at("name"));
          break;
...
o2::framework::WorkflowSpec InfrastructureGenerator::generateRemoteInfrastructure(std::string configurationSource)
{
  ...
  for (const auto& taskSpec : infrastructureSpec.tasks) {
    ...
    if (taskSpec.location == TaskLocationSpec::Local) {
      ...
      generateLocalTaskRemoteProxy(workflow, taskSpec.taskName, numberOfLocalMachines, std::to_string(taskSpec.remotePort));
      ...
      generateMergers(workflow, taskSpec.taskName, numberOfLocalMachines, cycleDurationSeconds, taskSpec.mergingMode, resetAfterCycles);
